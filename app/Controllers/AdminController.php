...
class AdminController extends Controller
{
    ...
    public function guardarHorario(): void
    {
        ...
        $db = \App\Core\Database::getInstance();
        $existente = $db->fetchOne("SELECT id_horario FROM horarios_operador WHERE id_empleado = :id", ['id' => $idEmpleado]);

        if ($existente) {
            $db->update('horarios_operador', [
                'turno' => $turno,
    ...
    public function autorizarAcceso(): void
    {
        ...
        $db->insert('accesos_extraordinarios', [
            'id_empleado' => $idEmpleado,
