...
class UsuariosController extends Controller
{
    ...
    public function index(): void
    {
        ...
        $data = [
            'usuarios' => $this->user->getAllWithEmpleado(),
            'pageTitle' => 'Usuarios',
            'readonly' => !es_admin(),
        ];
    ...
    public function store(): void
    {
        ...
        try {
            if ($idEmpleado) {
                $empleado = $db->fetchOne("SELECT id_empleado FROM empleados WHERE id_empleado = :id", ['id' => $idEmpleado]);
                if (!$empleado) {
            ...
            $exists = $db->fetchOne("SELECT id_usuario FROM usuarios WHERE nombre_usuario = :u", ['u' => $nombreUsuario]);
            if ($exists) {
    ...
    {
        $this->requireRol(1);
        $usuario = $this->user->getWithEmpleado($params['id']);
        if (!$usuario) {
